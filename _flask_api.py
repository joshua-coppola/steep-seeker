...
def explore_map():
    ...
    return render_template(
        "explore_map.jinja", nav_links=nav_links, active_page="explore", geojson=geojson
    )
...
def interactive_map(state, name):
    ...
    for trail in trails:
        ...
        popup_content += (
            f"<p>Length: {trail.length} ft</p><p>Vertical Drop: {trail.vertical} ft</p>"
        )
...
def site_map():
    ...
    static_page_priority = [1, 0.6, 0.7, 0.8, 0.9, 0.9, 0.8]
    dynamic_page_list = ["map", "interactive-map"]
    dynamic_page_priority = 0.3
    ...
    for page in dynamic_page_list:
        for mountain in database.get_mountains():
            xml += url.format(
                path=page + "/" + mountain[1] + "/" + mountain[0],
                priority=dynamic_page_priority,
            )
    xml += footer
