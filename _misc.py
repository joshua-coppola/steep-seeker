...
def fill_point_gaps(nodes: list(dict())) -> list(dict()):
    ...
    while gaps_exist:
        ...
        while i < len(nodes):
            ...
            if dist > max_gap:
                new_node = {
                    "lat": (point["lat"] + previous_point["lat"]) / 2,
    ...
    for node_list in hundred_node_lists:
        ...
        for node in node_list:
            if piped_coords == "":
                ...
                continue
            piped_coords = piped_coords + "|{},{}".format(node[0], node[1])
...
def get_steep_pitch(nodes: list(tuple()), length: float) -> float:
    ...
    for loc, node in enumerate(nodes):
        ...
        while i < len(nodes):
            ...
            if cumulative_dist >= length:
                ...
                slope = None
                if elevation_change != 0:
                    slope = abs(degrees(atan(elevation_change / cumulative_dist)))
                    if slope < 0:
                        print(slope)
                else:
                    slope = 0
    ...
    if max_pitch == -90:
        ...
        if length <= 30:
            ...
            if elevation_change != 0:
                max_pitch = abs(degrees(atan(elevation_change / trail_length(nodes))))
            else:
                max_pitch = 0
        else:
            return "NULL"
...
def mountain_rating(nodes: list()) -> tuple:
    ...
    difficulty = ((sum(nodes[0:divisor]) / divisor) * 0.2) + (
        (sum(nodes[0:5]) / small_divisor) * 0.8
    )

    beginner_friendliness = ((sum(nodes[-divisor:]) / divisor) * 0.2) + (
        (sum(nodes[-5:]) / small_divisor) * 0.8
    )
...
def find_corrected_center(
    center_lat: float, center_lon: float, nodes: list(tuple()), north_south: bool
) -> tuple():
    if north_south:
        # check lon
        dist = []
...
def process_area(nodes: list(tuple())) -> list(tuple()):
    ...
    new_nodes = []
    new_node = {"lat": nodes[max_elevation[1]][0], "lon": nodes[max_elevation[1]][1]}
    new_nodes.append(new_node)
    if max_elevation[0] in sub_region1_ele:
        new_node = {"lat": new_center_lat_1, "lon": new_center_lon_1}
        new_nodes.append(new_node)
    else:
        new_node = {"lat": new_center_lat_2, "lon": new_center_lon_2}
        new_nodes.append(new_node)
    new_node = {"lat": new_center_lat, "lon": new_center_lon}
    new_nodes.append(new_node)
    if max_elevation[0] in sub_region1_ele:
        new_node = {"lat": new_center_lat_2, "lon": new_center_lon_2}
        new_nodes.append(new_node)
        ...
        new_node = {"lat": new_center_lat_1, "lon": new_center_lon_1}
        new_nodes.append(new_node)
    new_node = {"lat": nodes[min_elevation[1]][0], "lon": nodes[min_elevation[1]][1]}
    new_nodes.append(new_node)
...
def get_weather(lat: float, lon: float):
    ...
    if response.status_code == 200:
        ...
        if "Daily API request limit exceeded. Please try again tomorrow." in str(
            response.content
        ):
            raise ValueError(
...
def process_weather(weather_list: list):
    ...
    for row in winter_list:
        if float(row["temperature_2m_max"]) > 34 and float(
            row["temperature_2m_min"] < 32
        ):
            freeze_thaw += 1
...
def get_weather_modifier(weather_dict: dict):
    ...
    modifier += (
        (weather_dict["icy_days"] - min_icy_days) / (max_icy_days - min_icy_days)
    ) * 2
