...
def refresh_resort_from_osm(name: str, state: str, area_padding: float = 0) -> None:
    ...
    lat_adj = (bbox[2] - bbox[0]) * area_padding * 0.5
    lon_adj = (bbox[3] - bbox[1]) * area_padding * 0.5
    ...
    if os.path.exists(f"data/osm/{state}/{name}.osm"):
        ...
        os.replace(
            f"data/osm/{state}/{name}.osm",
            f"data/osm-old/{state}/{old_date} {name}.osm",
        )
...
def rename_resort(old_name: str, state: str, new_name: str) -> None:
    ...
    if os.path.exists(f"data/osm/{state}/{old_name}.osm"):
        os.rename(
            f"data/osm/{state}/{old_name}.osm", f"data/osm/{state}/{new_name}.osm"
        )
    if os.path.exists(f"static/maps/{state}/{old_name}.svg"):
        os.rename(
            f"static/maps/{state}/{old_name}.svg", f"static/maps/{state}/{new_name}.svg"
        )
    if os.path.exists(f"static/thumbnails/{state}/{old_name}.svg"):
        os.rename(
            f"static/thumbnails/{state}/{old_name}.svg",
            f"static/thumbnails/{state}/{new_name}.svg",
        )
...
def prune_objects(
    resort_name: str, state: str, prune_id_list: list(), debug_mode: bool = True
) -> None:
    mountain_id = db.get_mountain_id(resort_name, state)
...
def repl() -> None:
    ...
    while valid:
        ...
        if operation == 4:
            ...
            extra_area = float(
                input(
                    "Enter Addition Search Area Ratio (0 is no change, 1 is double both dimensions): "
                )
            )
