        ...
        if in_way:
            ...
            if "nd" == parsed["class"]:
                ...
                if parsed["ref"] in nodes:
                    ...
                    print(
                        "Malformed node reference: {} not found".format(parsed["ref"])
                    )
...
def read_xml_string(string: str) -> str:
    ...
    for i, word in enumerate(words):
        ...
        if i == 0:
            ...
            output["class"] = re.sub(r"[^'A-Za-z0-9.\- ]+", " ", split_word[0]).strip()
            word = split_word[1]
        # adds values & removes weird characters
        output[re.sub(r"[^'A-Za-z0-9.\- ]+", " ", word).strip()] = re.sub(
            r"[^'A-Za-z0-9.\- ]+", " ", words[i + 1]
        ).strip()
    # returns dict
    return output
