...
def process_trails(ways: list(dict())) -> tuple():
    ...
    for unprocessed_way in ways:
        ...
        for tag in unprocessed_way["tags"]:
            ...
            if "piste type" in tag:
                if (
                    tag["piste type"] == "downhill"
                    or tag["piste type"] == "traverse"
                    or tag["piste type"] == "snow park"
                ):
                    way["type"] = "trail"
                elif tag["piste type"] == "yes":
                    print(
                        'Way #{} ({}) is marked as piste type "yes". Please make more specific.'.format(
                            way["id"], way["name"]
                        )
                    )
                    way["type"] = "trail"
            ...
            if "piste grooming" in tag:
                ...
                if (
                    "skating" in tag["piste grooming"]
                    or "scooter" in tag["piste grooming"]
                ):
                    way["type"] = None
                    way["valid"] = False
                if (
                    "backcountry" in tag["piste grooming"]
                    or "mogul" in tag["piste grooming"]
                    or "no" in tag["piste grooming"]
                ):
                    way["ungroomed"] = True
    ...
    if len(multiples) > 0:
        print(
            "The following trails have X number of ways named the same. Please see if they all make sense."
        )
    ...
    for i, trail in enumerate(trails):
        ...
        if trail["name"] in previously_seen_names:
            ...
            for j in indices:
                ...
                common_traits = 1
                if (
                    previously_seen_dicts[j]["official_rating"]
                ...
                if common_traits == 5:
                    ...
                    if (
                        previously_seen_dicts[j]["nodes"][-1]["lat"]
                        == trail["nodes"][0]["lat"]
                        and previously_seen_dicts[j]["nodes"][-1]["lon"]
                        == trail["nodes"][0]["lon"]
                    ):
                    ...
                    if (
                        previously_seen_dicts[j]["nodes"][0]["lat"]
                        == trail["nodes"][-1]["lat"]
                        and previously_seen_dicts[j]["nodes"][0]["lon"]
                        == trail["nodes"][-1]["lon"]
                    ):
